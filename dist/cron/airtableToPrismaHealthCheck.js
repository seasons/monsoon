...
function checkProductsAlignment() {
    return __awaiter(this, void 0, void 0, function* () {
        const allAirtableProductVariants = yield utils_1.getAllProductVariants();
        const allAirtablePhysicalProducts = yield utils_1.getAllPhysicalProducts();
        const allAirtableProducts = yield utils_1.getAllProducts();
        const allAirtableReservations = yield utils_1.getAllReservations();
        const allPrismaProductVariants = yield server_1.db.query.productVariants({}, `{
          id
          product {
              slug
          }
        ...
        const allPrismaPhysicalProducts = yield prisma_1.prisma.physicalProducts();
        const allPrismaProducts = yield prisma_1.prisma.products();
        const allPrismaReservations = yield server_1.db.query.reservations({}, `
    {
        id
        reservationNumber
        products {
    ...
    `);
        ...
        const allAirtableProductSlugs = allAirtableProducts.map(prod => prod.fields.Slug);
        const productsInAirtableButNotPrisma = allAirtableProductSlugs.filter(slug => !allPrismaProductSlugs.includes(slug));
        const productsInPrismaButNotAirtable = allPrismaProductSlugs.filter(slug => !allAirtableProductSlugs.includes(slug));
        /* Do we have any physical products in airtable but not prisma? Vice versa */
        const allPrismaPhysicalProductSUIDs = allPrismaPhysicalProducts.map(physProd => physProd.seasonsUID);
        const allAirtablePhysicalProductSUIDs = allAirtablePhysicalProducts.map(physProd => physProd.fields.SUID.text);
        const physicalProductsInAirtableButNotPrisma = allAirtablePhysicalProductSUIDs.filter(suid => !allPrismaPhysicalProductSUIDs.includes(suid));
        const physicalProductsInPrismaButNotAirtable = allPrismaPhysicalProductSUIDs.filter(suid => !allAirtablePhysicalProductSUIDs.includes(suid));
        /* Do we have any product variants in airtable but not prisma? Vice versa? */
        const allPrismaProductVariantSKUs = allPrismaProductVariants.map(prodVar => prodVar.sku);
        const allAirtableProductVariantSKUs = allAirtableProductVariants.map(prodVar => prodVar.fields.SKU);
        const productVariantsInAirtableButNotPrisma = allAirtableProductVariantSKUs.filter(sku => !allPrismaProductVariantSKUs.includes(sku));
        const productVariantsInPrismaButNotAirtable = allPrismaProductVariantSKUs.filter(sku => !allAirtableProductVariantSKUs.includes(sku));
        ...
        */
        const { productVariantSKUMismatches, errors: prismaAirtableSKUCheckErrors, } = getPrismaAirtableProductVariantSKUMismatches(allAirtableProducts, allAirtableProductVariants, allPrismaProductVariants, productVariantsInPrismaButNotAirtable);
        errors = [...errors, ...prismaAirtableSKUCheckErrors];
        /* Check SUIDs. Are they correct on prisma? Are the correct on airtable? */
        const { prismaSUIDToSKUMismatches, airtableSUIDToSKUMismatches } = checkSUIDs(allPrismaProductVariants, allAirtableProductVariants, allAirtablePhysicalProducts);
        /* Are the product variant counts matching between prisma and airtable? */
        const { countMisalignments, prismaTotalPhysicalProductMisalignment, airtableTotalPhysicalProductMisalignment, } = checkCounts(allAirtableProductVariants, allPrismaProductVariants, allAirtableProducts);
        const [prismaCountToStatusMisalignments, airtableCountToStatusMisalignments,] = checkMisalignmentsBetweenProdVarCountsAndPhysProdStatuses(allPrismaProductVariants, allAirtableProductVariants, allAirtablePhysicalProducts);
        const prismaProdVarsWithImpossibleCounts = yield getProdVarsWithImpossibleCounts(allPrismaProductVariants);
        /* Are the physical product statuses matching between prisma and airtable? */
        const { mismatchingStatuses, physicalProductsOnPrismaButNotAirtable, } = checkPhysicalProductStatuses(allPrismaPhysicalProducts, allAirtablePhysicalProducts);
        /* Are the reservations aligned? */
        const { misalignedSUIDsOnReservations, misalignedStatusOnReservations, reservationsWithMoreThanThreeProducts, reservationsInAirtableButNotPrisma, reservationsInPrismaButNotAirtable, } = checkReservations(allPrismaReservations, allAirtableReservations, allAirtablePhysicalProducts);
        /* REPORT */
        console.log(`/*********** REPORT ***********/`);
...
function getPrismaAirtableProductVariantSKUMismatches(allAirtableProducts, allAirtableProductVariants, allPrismaProductVariants, productVariantsInPrismaButNotAirtable) {
    ...
    for (const prismaProductVariant of allPrismaProductVariants) {
        try {
            ...
            const correspondingAirtableProductVariant = utils_2.getCorrespondingAirtableProductVariant(allAirtableProducts, allAirtableProductVariants, prismaProductVariant);
...
function checkSUIDs(allPrismaProductVariants, allAirtableProductVariants, allAirtablePhysicalProducts) {
    ...
    for (const airtableProductVariant of allAirtableProductVariants) {
        ...
        for (const airtablePhysProdRecordID of airtableProductVariant.fields["Physical Products"]) {
            const airtablePhysProdRecord = allAirtablePhysicalProducts.find(rec => rec.id == airtablePhysProdRecordID);
...
function checkCounts(allAirtableProductVariants, allPrismaProductVariants, allAirtableProducts) {
    ...
    for (const prismaProductVariant of allPrismaProductVariants) {
        ...
        const totalCorrect = prismaProductVariant.total ===
            correspondingAirtableProductVariant.fields["Total Count"];
        const reservableCorrect = prismaProductVariant.reservable ===
            correspondingAirtableProductVariant.fields["Reservable Count"];
        const reservedCorrect = prismaProductVariant.reserved ===
            correspondingAirtableProductVariant.fields["Reserved Count"];
        const nonReservableCorrect = prismaProductVariant.nonReservable ===
        ...
        const noPhysicalProductsAndMisalignment = !correspondingAirtableProductVariant.fields["Physical Products"] &&
            correspondingAirtableProductVariant.fields["Total Count"] !== 0;
        const physicalProductsAndMisalignment = !!correspondingAirtableProductVariant.fields["Physical Products"] &&
...
function checkPhysicalProductStatuses(allPrismaPhysicalProducts, allAirtablePhysicalProducts) {
    ...
    for (const prismaPhysicalProduct of allPrismaPhysicalProducts) {
        const correspondingAirtablePhysicalProduct = utils_2.getCorrespondingAirtablePhysicalProduct(allAirtablePhysicalProducts, prismaPhysicalProduct);
...
function checkReservations(allPrismaReservations, allAirtableReservations, allAirtablePhysicalProducts) {
    ...
    const allPrismaReservationNumbers = allPrismaReservations.map(resy => resy.reservationNumber);
    const allAirtableReservationNumbers = allAirtableReservations.map(resy => resy.fields.ID);
    const reservationsInPrismaButNotAirtable = allPrismaReservationNumbers.filter(prismaResyNumber => !allAirtableReservationNumbers.includes(prismaResyNumber));
    const reservationsInAirtableButNotPrisma = allAirtableReservationNumbers.filter(airtableResyNumber => !allPrismaReservationNumbers.includes(airtableResyNumber));
    for (const prismaResy of allPrismaReservations) {
        if (reservationsInPrismaButNotAirtable.includes(prismaResy.reservationNumber)) {
            continue;
        }
        const correspondingAirtableReservation = allAirtableReservations.find(airtableResy => airtableResy.fields.ID === prismaResy.reservationNumber);
        // Check SUID match
        const prismaPhysicalProductSUIDs = prismaResy.products.map(prod => prod.seasonsUID);
        const airtablePhysicalProductSUIDs = correspondingAirtableReservation.fields.Items.map(airtablePhysicalProductRecordID => allAirtablePhysicalProducts.find(airtablePhysProd => airtablePhysProd.id === airtablePhysicalProductRecordID)).map(airtablePhysProdRecord => airtablePhysProdRecord.fields.SUID.text);
...
const checkMisalignmentsBetweenProdVarCountsAndPhysProdStatuses = (allPrismaProductVariants, allAirtableProductVariants, allAirtablePhysicalProducts) => {
    const prismaMisalignments = allPrismaProductVariants
        .filter(a => {
        const physicalProductsWithStatusReserved = a.physicalProducts.filter(b => b.inventoryStatus === "Reserved");
        const physicalProductsWithStatusReservable = a.physicalProducts.filter(b => b.inventoryStatus === "Reservable");
        const physicalProductsWithStatusNonReservable = a.physicalProducts.filter(b => b.inventoryStatus === "NonReservable");
        ...
        physicalProducts: c.physicalProducts.map(d => utils_3.Identity({
            suid: d.seasonsUID,
    ...
    const airtableMisalignments = allAirtableProductVariants
        .filter(a => {
        const correspondingAirtablePhysicalProducts = getAttachedAirtablePhysicalProducts(allAirtablePhysicalProducts, a);
        const physicalProductsWithStatusReserved = correspondingAirtablePhysicalProducts.filter(c => c.fields["Inventory Status"] === "Reserved");
        const physicalProductsWithStatusReservable = correspondingAirtablePhysicalProducts.filter(c => c.fields["Inventory Status"] === "Reservable");
        const physicalProductsWithStatusNonReservable = correspondingAirtablePhysicalProducts.filter(c => c.fields["Inventory Status"] === "Non Reservable");
        return (!!a.fields.SKU &&
        ...
        physicalProducts: getAttachedAirtablePhysicalProducts(allAirtablePhysicalProducts, d).map(e => utils_3.Identity({
...
const getAttachedAirtablePhysicalProducts = (allAirtablePhysicalProducts, airtableProductVariant) => {
    if (!airtableProductVariant.fields.SKU)
        return [];
    return allAirtablePhysicalProducts.filter(a => airtableProductVariant.fields["Physical Products"].includes(a.id));
};
const getProductVariantReservationHistory = (prisma, prodVarSku) => __awaiter(void 0, void 0, void 0, function* () {
    return yield prisma.reservations({
        where: { products_some: { seasonsUID_contains: prodVarSku } },
    });
});
const setProductVariantCountsByPhysicalProductStatuses = (prodVar) => __awaiter(void 0, void 0, void 0, function* () {
    const trueReserved = prodVar.physicalProducts.filter(b => b.status === "Reserved").length;
    const trueReservable = prodVar.physicalProducts.filter(b => b.status === "Reservable").length;
    const trueNonReservable = prodVar.physicalProducts.filter(b => b.status === "NonReservable").length;
    const trueCounts = {
...
const getProdVarsWithImpossibleCounts = (allPrismaProductVariants) => __awaiter(void 0, void 0, void 0, function* () {
    return allPrismaProductVariants
        .filter(a => a.total !== a.reserved + a.reservable + a.nonReservable)
        .map(a => utils_3.Identity({
        sku: a.sku,
        total: a.total,
        reserved: a.reserved,
...
const alignAirtableCountsWithPrismaCounts = (allAirtableProductVariants, allPrismaProductVariants) => __awaiter(void 0, void 0, void 0, function* () {
    allPrismaProductVariants.forEach((a) => __awaiter(void 0, void 0, void 0, function* () {
        const correspondingAirtableProductVariant = allAirtableProductVariants.find(b => !!b.fields.SKU && b.fields.SKU === a.sku);
        yield new Promise(resolve => setTimeout(resolve, 1000));
        yield updateProductVariantCounts_1.updateProductVariantCounts(correspondingAirtableProductVariant.id, {
