...
const MODELS_TO_SANITIZE = uniq([
  ...Object.keys(JSON_FIELD_NAMES),
  ...Object.keys(SINGLETON_RELATIONS_POSING_AS_ARRAYS),
  ...Object.keys(SCALAR_LIST_FIELD_NAMES),
])
@Injectable()
  ...
  */
  sanitizePayload<T>(payload: T, modelName: Prisma.ModelName): T {
    ...
    singleRelationFieldNames.forEach(fieldName => {
      const fieldInPayload = !!payload[fieldName]
      if (!!fieldInPayload) {
        const valueType = PrismaService.modelFieldsByModelName[modelName].find(
          a => a.name === fieldName
        ).type
        returnPayload[fieldName] = this.sanitizePayload(
          head(payload?.[fieldName]),
          valueType
        )
      }
    })
    ...
    modelFields.forEach(field => {
      ...
      if (
        fieldInPayload &&
        field.kind === "object" &&
        !singleRelationFieldNames.includes(field.name) &&
        !scalarListFieldNames.includes(field.name)
      ) {
        returnPayload[field.name] = this.sanitizePayload(
          payload[field.name],
          field.type
        )
      }
  ...
  private getAllTypes(payload: any, payloadType: string): string[] {
    ...
    for (const k of Object.keys(payload)) {
      let typeOfK
      try {
        typeOfK = PrismaService.modelFieldsByModelName[payloadType].find(
          a => a.name === k
        ).type
      } catch (e) {
