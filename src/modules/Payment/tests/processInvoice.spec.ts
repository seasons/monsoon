...
describe("Charge customer", () => {
  ...
  describe("Properly charges an access-monthly customer", () => {
    ...
    let actualResultsDict
    let expectedResultsDict
    ...
    beforeAll(async () => {
      ...
      const rentalInvoice = customer.membership.rentalInvoices[0]
      // Create line items.
      // Mock createRentalInvoiceLineItems to return those
      // Set the customer plan type.
      // Call process invoice and see what happened.
      const lineItemDatas = [
        {
          price: 30,
          currencyCode: "USD",
          rentalInvoiceId: rentalInvoice.id,
          id: cuid(),
          name: "test1",
        },
        {
      ...
      jest
        .spyOn(rentalService, "createRentalInvoiceLineItems")
        .mockResolvedValue(lineItemDatas.map(a => ({ id: a.id })) as any)

      await setCustomerPlanTypeWithParams("access-monthly")
      ;({
        lineItems,
        charges: addedCharges,
      } = await rentalService.processInvoice(rentalInvoice))
      billedRentalInvoice = await prisma.client.rentalInvoice.findUnique({
        where: { id: rentalInvoice.id },
        select: { id: true, status: true },
      })

      const custWithUpdatedData = (await testUtils.getCustWithData(customer, {
        membership: {
          select: {
            rentalInvoices: {
              select: { status: true },
      ...
      actualResultsDict = createLineItemHash(lineItemsWithDataAfterCharging)
      expectedResultsDict = {
        test1: {
          taxPrice: 240,
          taxRate: 0.08,
    ...
    it("Adds taxes to the line items", () => {
      expect(Object.keys(expectedResultsDict).length).toBe(2)
      for (const id of Object.keys(expectedResultsDict)) {
        expect(actualResultsDict[id].taxPrice).toEqual(
          expectedResultsDict[id].taxPrice
        )
        expect(actualResultsDict[id].taxRate).toEqual(
          expectedResultsDict[id].taxRate
        )
      }
    })
  })

  // xdescribe("Properly charges an access-annual customer", () => {
  //   let addedCharges
  //   let lineItemsWithDataAfterCharging
  ...
  // xdescribe("Properly handles an error from chargebee", () => {
  //   beforeAll(async () => {
  //     const spy = jest
  //       .spyOn<any, any>(chargebee.subscription, "add_charge_at_term_end")
  //       .mockReturnValue(
  ...
  // xdescribe("Properly handles an error when creating line items", () => {
  //   beforeAll(async () => {
  //     const spy = jest
  ...
  // xdescribe("Properly handles retries of invoices with status ChargeFailed", () => {
  //   let custWithData
