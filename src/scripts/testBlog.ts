...
const run = async () => {
  ...
  const lastPostStored = await ps.client2.blogPost.findFirst({
    orderBy: {
  ...
  const blogImage = await ps.client2.image.upsert({
    where: { url: imageData.url },
    create: { ...imageData, title, alt },
    update: { ...imageData, title, alt },
  })

  const newBLog = await ps.client2.blogPost.update({
    where: { id: lastPostStored.id },
    data: {
      ...queryUtils.prismaOneToPrismaTwoMutateData(
        blogData,
        null,
        "BlogPost",
        "update"
